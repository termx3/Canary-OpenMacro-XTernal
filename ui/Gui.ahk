...
GetGui() {
    ...
    MainTab.UseTab(4)
        ...
        mg.AddText("x15 y95 w370 h300 c" TextColor,
            "• Fix Roblox (F2) now checks running Roblox version against latest before re-attaching — warns if offsets may be outdated`n"
            . "• GitHub update check is now cached for 5 minutes — prevents repeated network requests on frequent reloads`n"
            . "• Batch updater escape logic hardened to handle all cmd metacharacters`n"
            . "• Settings tab now includes an Open Folder button to access the XTernal AppData directory`n"
            . "• All dialogs (update, post-update, add mutation) now respect the active appearance theme`n"
            . "• Update dialog no longer allows double-clicking Download; install is deferred until dialog hides cleanly`n"
            . "• Post-update dialog now blocks until dismissed`n"
            . "• Pointing to Canary release channel")
            .SetFont("s10")
